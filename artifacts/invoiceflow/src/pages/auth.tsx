...
export default function AuthPage({ isLogin = true }: { isLogin?: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulate auth, redirect to dashboard which checks profile
    setLocation("/dashboard");
  };

  return (
    <div className="min-h-screen bg-muted/30 flex flex-col items-center justify-center p-4">
      <Link href="/" className="flex items-center gap-3 mb-8 hover-elevate">
        <img src={`${import.meta.env.BASE_URL}images/logo.png`} alt="Logo" className="w-10 h-10" />
        <span className="font-display font-bold text-2xl">InvoiceFlow</span>
      </Link>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <Card className="border-border/50 shadow-xl shadow-black/5 rounded-2xl">
          <CardHeader className="text-center space-y-2 pb-6">
            <CardTitle className="text-3xl font-display">{isLogin ? "Welcome back" : "Create an account"}</CardTitle>
            <CardDescription className="text-base">
              {isLogin ? "Enter your details to access your dashboard" : "Start generating invoices in minutes"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="hello@example.com" required className="h-12 rounded-xl" />
              </div>
              <div className="space-y-2">
              ...
              <Button type="submit" className="w-full h-12 rounded-xl text-base font-semibold shadow-lg shadow-primary/20 hover-elevate">
            ...
            <div className="mt-6">
              ...
              <Button variant="outline" className="w-full h-12 rounded-xl mt-6 hover-elevate" onClick={() => setLocation("/dashboard")}>
          ...
          <CardFooter className="flex justify-center border-t py-6 bg-secondary/30 rounded-b-2xl">
